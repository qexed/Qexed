                ...
                tokio::spawn(async move {
                    if let Some(socket) = socket {
                        ...
                        let _: anyhow::Result<()> = async {
                            ...
                            // 发送初始任务到游戏逻辑
                            ReturnMessage::build(qtunnel_server_logic::message::TaskMessage::Start(player, Some(rpr), Some(wpw.clone())))
                                .get(&logic_api).await?;
                            ReturnMessage::build(qtunnel_server_logic::message::TaskMessage::Configuration(false))
                                .get(&logic_api).await?;
                            ReturnMessage::build(qtunnel_server_logic::message::TaskMessage::Play)
                                .get(&logic_api).await?;
                            // 等待任一任务完成，然后协调关闭
                            tokio::select! {
                        ...
                        if let Some(logic_api) = qexed_logic_api{
                            ...
                            let _ = ReturnMessage::build(qtunnel_server_logic::message::TaskMessage::Close)
                                .get(&logic_api)
                                .await;
                        }
...
async fn login_status(
    ...
    public_key_der: Vec<u8>,
    addr: std::net::SocketAddr
) -> anyhow::Result<(qexed_player::Player,Option<UnboundedSender<ReturnMessage<qtunnel_server_logic::message::TaskMessage>>>)> {
    let mut player: qexed_player::Player = Default::default();
    let mut verify_token: Option<[u8; 16]> = None;
    let mut encryption_started = false;
    let mut logic_api: Option<UnboundedSender<ReturnMessage<qtunnel_server_logic::message::TaskMessage>>>=None;
    loop {
        ...
        match id.0 {
            0x00 => {
                ...
                // 非online模式的话其实就结束了
                if !online_mode {
                    if let ManagerCommand::GetLogicApi(qtunnel_server_logic::message::ManagerMessage::NewPlayerConnect(uuid, is_true, err, logic_api2)) = ReturnMessage::build(ManagerCommand::GetLogicApi(
                        qtunnel_server_logic::message::ManagerMessage::NewPlayerConnect(
