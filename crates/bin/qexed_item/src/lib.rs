...
mod tests {
    ...
    fn test_item_id_conversion() {
        // 测试ID转换
        let stone = ItemId::Stone;
        assert_eq!(stone.to_i32(), 1);
        assert_eq!(stone.to_u32(), 1);
        
        // 测试从ID解析
        let from_id = ItemId::try_from(1u32);
        assert!(from_id.is_ok());
        assert_eq!(from_id.unwrap(), ItemId::Stone);
        
        // 测试从i32解析
        let from_i32 = ItemId::try_from(1i32);
        assert!(from_i32.is_ok());
        assert_eq!(from_i32.unwrap(), ItemId::Stone);
    }
    ...
    fn test_item_registry() {
        ...
        // 测试有效ID
        assert!(registry.is_valid_item_id(0)); // Air
        assert!(registry.is_valid_item_id(1)); // Stone
        
        // 测试无效ID
        assert!(!registry.is_valid_item_id(999999));
        
        // 测试获取信息
        let stone_info = registry.get_item_info(1);
        assert!(stone_info.is_some());
        
        if let Some(info) = stone_info {
            assert_eq!(info.id, 1);
            assert!(info.name.contains("stone"));
        }
    }
    
    #[test]
    fn test_item_inventory()->anyhow::Result<()> {
        let mut inventory = ItemInventory::new(36);
        
        // 测试添加物品
        let remaining= inventory.add_item(ItemId::Stone, 65)?;
        assert_eq!(remaining, 1); // 64个堆叠，剩下1个
        
        // 检查第一个槽位
        let slot = inventory.get_slot(0);
        assert!(slot.is_some());
        
        if let Some(slot) = slot {
            assert_eq!(slot.item, ItemId::Stone);
            assert_eq!(slot.count, 64);
        }
        
        // 检查第二个槽位
        let slot = inventory.get_slot(1);
        assert!(slot.is_some());
    ...
    #[test]
    fn test_all_items_iteration() {
        // 测试可以迭代所有物品
        let items: Vec<ItemId> = all_items();
        assert!(!items.is_empty());
        
        // 确保包含默认物品
        assert!(items.contains(&DEFAULT_ITEM));
    }
}