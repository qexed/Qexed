use log;
use qexed_config::{app::qtunnel_one::One, tool::AppConfigTrait};
...
async fn main() -> anyhow::Result<()> {
    ...
    if let qexed_command::message::ManagerCommand::RegisterCommand{success, .. } =
        ReturnMessage::build(qexed_command::message::ManagerCommand::RegisterCommand{
            name:"qstop".to_string(),
            doc:"关闭代理端".to_string(),
            permission:"qtunnel.console.stop".to_string(),
            parameters: vec![],
            aliases:vec![],
            api:Some(cmd_tx),
            success:false,
            ...
            while let Some(cmd) = cmd_rx.recv().await {
                ...
                if cmd.is_cmd {
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            // 发送聊天消息失败，记录日志但继续运行
                            log::error!("[指令] [qstop] 无法向玩家发送无权限提示: {}",  e);
                        }
                    };
    ...
    let cli_result = qexed_cli::run_cli(command_tx, Some(log_rx),qexed_config::QTUNNEL_NAME).await;
