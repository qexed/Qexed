...
use qexed_protocol::{to_client::{ play::system_chat::SystemChat}, to_server::play::{keep_alive::KeepAlive, pong::Pong}};
use qexed_task::{
...
pub fn build_dirt_from_4_stones_recipe() -> qexed_protocol::to_client::play::recipe_book_add::RecipeBookAdd {
    ...
    let dirt_id = qexed_packet::net_types::VarInt(28);       // 泥土
    
    // 创建无序合成 (2x2背包合成)
    let crafting_shapeless = qexed_protocol::types::minecraft::CraftingShapeless {
        ingredients: vec![
            qexed_protocol::types::SlotDisplay::Item(qexed_protocol::types::slot_display_types::minecraft::Item {
                item_type: stone_id.clone(),
            })
        ],
        
        // 合成结果：1个泥土
        result: qexed_protocol::types::SlotDisplay::Item(qexed_protocol::types::slot_display_types::minecraft::Item {
    ...
    qexed_protocol::to_client::play::recipe_book_add::RecipeBookAdd {
        entries: vec![
            qexed_protocol::to_client::play::recipe_book_add::Recipes {
                // 配方ID
                recipe: qexed_packet::net_types::VarInt(1002),  // 注意：使用不同的ID
                
                // 显示信息 - 使用无序合成
                display: qexed_protocol::types::RecipeDisplay::MinecraftCraftingShapeless(crafting_shapeless),
...
pub fn build_item()->anyhow::Result<qexed_protocol::to_client::play::container_set_content::ContainerSetContent>{
    let total_slots = 46;
    let mut slot_data = vec![qexed_protocol::types::Slot::default(); total_slots];
    slot_data[0] = create_slot(&qexed_packet::net_types::VarInt(1), 16)?;  // 16个石头
    
    // 槽位1：泥土
    slot_data[1] = create_slot(&qexed_packet::net_types::VarInt(28), 10)?;  // 10个泥土
    
    // 在背包（9-35）的前4个格子放入石头
    for i in 9..13 {
        slot_data[i] = create_slot(&qexed_packet::net_types::VarInt(1), 1)?;  // 每个格子1个石头
    }
    Ok(qexed_protocol::to_client::play::container_set_content::ContainerSetContent{
...
/// 创建一个物品槽位
fn create_slot(item_id: &qexed_packet::net_types::VarInt, count: i8) -> anyhow::Result<qexed_protocol::types::Slot> {
    if count < 0 || count > 64 {
        return Err(anyhow::anyhow!("物品数量必须在0-64之间: {}", count));
    }
    
    Ok(qexed_protocol::types::Slot {
        item_count: qexed_packet::net_types::VarInt(count.into()),  // 数量为1
        item_id: Some(qexed_packet::net_types::VarInt(count.into())),  // 泥土的物品ID，假设为3（需要确认实际ID）
