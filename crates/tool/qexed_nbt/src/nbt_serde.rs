...
mod tests {
    ...
    #[test]
    fn test_struct_serialization() -> Result<(), NbtError> {
        let mut map = HashMap::new();
        ...
        // 序列化
        let tag = nbt_serde::to_tag(&test_struct)?;
        
        // 反序列化
        let deserialized: TestStruct = nbt_serde::from_tag(&tag)?;
    ...
    #[test]
    fn test_enum_serialization() -> Result<(), NbtError> {
        // 测试 UnitVariant
        let unit_enum = TestEnum::UnitVariant;
        let tag = nbt_serde::to_tag(&unit_enum)?;
        let deserialized: TestEnum = nbt_serde::from_tag(&tag)?;
        assert_eq!(unit_enum, deserialized);
        
        // 测试 NewtypeVariant
        let newtype_enum = TestEnum::NewtypeVariant(42);
        let tag = nbt_serde::to_tag(&newtype_enum)?;
        let deserialized: TestEnum = nbt_serde::from_tag(&tag)?;
        assert_eq!(newtype_enum, deserialized);
        
        // 测试 TupleVariant
        let tuple_enum = TestEnum::TupleVariant(1, "test".to_string());
        let tag = nbt_serde::to_tag(&tuple_enum)?;
        let deserialized: TestEnum = nbt_serde::from_tag(&tag)?;
        assert_eq!(tuple_enum, deserialized);
        
        // 测试 StructVariant
        let struct_enum = TestEnum::StructVariant { x: 10, y: 20 };
        let tag = nbt_serde::to_tag(&struct_enum)?;
        let deserialized: TestEnum = nbt_serde::from_tag(&tag)?;
        assert_eq!(struct_enum, deserialized);
        
        Ok(())
    }
    
    #[test]
    fn test_byte_array() -> Result<(), NbtError> {
        #[derive(Debug, Serialize, Deserialize, PartialEq)]
        struct WithBytes {
            data: Vec<u8>,
        }
        ...
        let tag = nbt_serde::to_tag(&original)?;
        let deserialized: WithBytes = nbt_serde::from_tag(&tag)?;
    ...
    #[test]
    fn test_named_root() -> Result<(), NbtError> {
        #[derive(Debug, Serialize, Deserialize, PartialEq)]
        struct SimpleStruct {
            value: i32,
        }
        ...
        root_map.insert("TestRoot".to_string(), nbt_serde::to_tag(&original)?);
        let root_tag = Tag::Compound(Arc::new(root_map));
        
        // 从命名标签反序列化
        let deserialized: SimpleStruct = nbt_serde::from_named_tag("TestRoot", &root_tag)?;
